...
public class BusStopLocationService {
     ...
     */
//    @Cacheable(cacheNames = "location", key = "#arsId", unless = "#result == null")
    public Optional<BusStopLocationDTO> selectBusStopLocation(String arsId) {
        try {
            return cacheService.getCacheOptional("location:" + arsId,
                    () -> getBusStopLocationDB(arsId), 1, TimeUnit.HOURS);
        } catch (Exception e) {
            log.error("ARS ID로 버스 정류장 위치 정보 조회 실패: {}", arsId, e);
            throw new ServiceException("ARS ID로 버스 정류장 위치 정보 조회 실패: " + e.getMessage(), SEARCH_FAIL);
        }
    }

    private Optional<BusStopLocationDTO> getBusStopLocationDB(String arsId) {
        return busStopLocationRepository.findByArsId(arsId).map(busStopLocation -> BusStopLocationDTO.builder()
