...
//    @Cacheable(cacheNames = "near", key = "#tmX.toString() + '_' + #tmY.toString() + '_' + #radius")
    public CommonApiDTO<ItemNear> getNearByBusStopApi(double tmX, double tmY, int radius) {
        ...
        CommonApiDTO<ItemNear> commonApiDTO = cacheService.getCache("near:" + tmX + "_" + tmY + "_" + radius,
                () -> getBusGoKrParser(apiResult), 1, TimeUnit.HOURS);
        String headerMsg = commonApiDTO.getMsgHeader().getHeaderMsg();
    ...
    private CommonApiDTO<ItemNear> getBusGoKrParser(String apiResult) {
